...
def bisection_method(func, tolerance=1e-5, max_iterations=100):
    ...
    if lambda_func(a) * lambda_func(b) >= 0:
        print("O teorema de Bolzano não é aplicável no intervalo fornecido.")
        return None
...
def newton_raphson_method(func, tolerance=1e-5, max_iterations=100):
    ...
    for i in range(max_iterations):
        ...
        if abs(f_prime_x) < tolerance:
            print("Derivada próxima de zero. O método não convergiu.")
            return None
...
def secant_method(func, tolerance=1e-5, max_iterations=100):
    ...
    for i in range(max_iterations):
        ...
        if abs(f_x1 - f_x0) < tolerance:
            print("Diferença próxima de zero. O método não convergiu.")
            return None
